...
class TestOrderFeed:
    ...
    @allure.title("Заказ пользователя из истории отображается в ленте заказов")
    def test_user_order_from_history_is_present_in_feed(
        ...
        assert feed_page.wait_for_order(order_number)

    @allure.title("После создания заказа увеличивается счётчик «Выполнено за всё время»")
    def test_create_order_increases_total_done_counter(self, driver, logged_in_user):
        feed_page = OrderFeedPage(driver)
        main_page = MainPage(driver)
    ...
    @allure.title("После создания заказа увеличивается счётчик «Выполнено за сегодня»")
    def test_create_order_increases_today_done_counter(self, driver, logged_in_user):
        feed_page = OrderFeedPage(driver)
        main_page = MainPage(driver)
    ...
    def test_created_order_appears_in_progress(self, driver, logged_in_user):
        main_page = MainPage(driver)
        feed_page = OrderFeedPage(driver)
